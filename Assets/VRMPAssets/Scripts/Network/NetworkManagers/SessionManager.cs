...
namespace XRMultiplayer
{
    ...
    public class SessionManager : MonoBehaviour
    {
        ...
        public const string k_RoomTypeKeyIdentifier = "rt";
        ...
        {
            return await CreateSession(roomName, isPrivate, maxPlayers, null);
        }

        public async Task<ISession> CreateSession(string roomName, bool isPrivate, int maxPlayers, Dictionary<string, SessionProperty> extraSessionProperties)
        {
            ...
            try
            {
                ...
                try
                {
                    var options = GetSessionOptions(sessionName, isPrivate, maxPlayers, extraSessionProperties).WithDistributedAuthorityNetwork();
        ...
        SessionOptions GetSessionOptions(string lobbyName, bool isPrivate = false, int maxPlayers = XRINetworkGameManager.maxPlayers, Dictionary<string, SessionProperty> extraSessionProperties = null)
        {
            ...
            {
                foreach (var pair in extraSessionProperties)
                {
                    if (string.IsNullOrWhiteSpace(pair.Key) || pair.Value == null)
                        continue;

                    sessionProperties[pair.Key] = pair.Value;
                }
            }
        ...
        public static bool TryReadSessionProperty(ISession session, string key, out string value)
        {
            value = null;

            if (session == null || string.IsNullOrWhiteSpace(key) || session.Properties == null)
                return false;
        ...
        public bool TryGetCurrentSessionProperty(string key, out string value)
        {
            return TryReadSessionProperty(m_CurrentSession, key, out value);
        }

        public string GetCurrentRoomTypeIdOrDefault(string fallbackRoomTypeId = "default")
        {
            if (TryGetCurrentSessionProperty(k_RoomTypeKeyIdentifier, out string roomTypeId))
                return roomTypeId;
        ...
        public async Task LeaveSession()
        {
            if (m_CurrentSession != null)
            {
                ...
                {
                    Utils.LogWarning($"{k_DebugPrepend}LeaveSession warning: {e.Message}");
                }
                finally
                {
            ...
            {
                Utils.Log($"{k_DebugPrepend}LeaveSession forcing NGO shutdown cleanup.", 0);
        ...
        public async Task<ISession> JoinLobbyById(string sessionId)
        {
            try
            {
                m_Status.Value = "Connecting To Friend Session";
                StopAllCoroutines();
                StartCoroutine(PlayConnectionMessage());

                m_CurrentSession = await Unity.Services.Multiplayer.MultiplayerService.Instance.JoinSessionByIdAsync(sessionId);
            ...
            {
                string failureMessage = "Failed to join friend's session.";
                Utils.Log($"{k_DebugPrepend}{failureMessage}\n\n{e}", 1);
                OnSessionFailed?.Invoke(failureMessage);
                return null;
            }
