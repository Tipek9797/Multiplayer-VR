...
namespace XRMultiplayer.MiniGames
{
    ...
    public class MiniGameManager : NetworkBehaviour
    {
        ...
        [SerializeField] Vector3 m_InGameOffset;
        [SerializeField] Vector3 m_PreGameOffset;
        ...
        void Start()
        {
            ...
            if (m_TeleportZonesObject != null)
                m_TeleportZonesObject.SetActive(false);

            if (m_BestAllText != null)
                m_BestAllText.text = "<b>Current Record</b>: No Record Set";

            if (currentMiniGame != null && m_GameNameText != null)
        ...
        void SetPreGameState()
        {
            ...
            if (m_GameStateText != null)
                m_GameStateText.text = "Pre Game";

            if (m_DynamicButton != null)
        ...
        void SetInGameState()
        {
            ...
            foreach (var trigger in m_StartZoneTrigger)
            {
                if (trigger != null && trigger.subTriggerCollider != null)
            ...
            {
                if (m_DynamicButton != null && m_DynamicButton.button != null)
                    m_DynamicButton.button.interactable = true;

                PlayerHudNotification.Instance.ShowText("Game Started!");
                ToggleShrink(true);

                if (m_UseInGameOffset && m_ScoreboardTransform != null && m_ScoreboardInGameTransform != null)
                {
                    m_ScoreboardTransform.rotation = m_ScoreboardInGameTransform.rotation;

                    Vector3 targetPosition =
                        m_ScoreboardInGameTransform.position +
                        (m_ScoreboardTransform.right * m_InGameOffset.x) +
                        (m_ScoreboardTransform.up * m_InGameOffset.y) +
                        (m_ScoreboardTransform.forward * m_InGameOffset.z);

                    m_ScoreboardTransform.position = targetPosition;
                }
            }
            else
            {
        ...
        void AddPlayerRpc(ulong clientId)
        {
            if (currentPlayerDictionary.Count < maxAllowedPlayers)
            {
                ...
                if (clientId == XRINetworkPlayer.LocalPlayer.OwnerClientId)
                {
                    ...
                    if (m_LocalPlayerTeleportProvider != null)
                    ...
                    if (destination != null && m_ScoreboardTransform != null)
                    {
                        m_ScoreboardTransform.rotation = destination.rotation;

                        Vector3 destinationPosition =
                            destination.position +
                            (m_ScoreboardTransform.right * m_PreGameOffset.x) +
                            (m_ScoreboardTransform.up * m_PreGameOffset.y) +
                            (m_ScoreboardTransform.forward * m_PreGameOffset.z);

                        m_ScoreboardTransform.position = destinationPosition;
                    }
        ...
        void UpdateScoreboardPosition()
        {
            ...
            Vector3 destinationPosition = destination.position
                + (m_ScoreboardTransform.right * offset.x)
                + (m_ScoreboardTransform.up * offset.y)
                + (m_ScoreboardTransform.forward * offset.z);

            m_ScoreboardTransform.position = Vector3.Lerp(
        ...
        Transform GetClosestReadyPosition(Vector3 position)
        {
            ...
            foreach (var readyZone in m_StartZoneTrigger)
            {
                if (closestTransform == null ||
                    Vector3.Distance(readyZone.transform.position, position) < Vector3.Distance(closestTransform.position, position))
                {
